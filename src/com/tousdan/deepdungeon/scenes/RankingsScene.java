...
import com.tousdan.deepdungeon.Rankings;
...
import com.tousdan.deepdungeon.windows.WndError;
import com.tousdan.deepdungeon.windows.WndRanking;
...
public class RankingsScene extends PixelScene {
	...
	public void create() {
		...
		if (Rankings.INSTANCE.records.size() > 0) {
			...
			BitmapText title = createText( TXT_TITLE, 9 );
			...
			if (Rankings.INSTANCE.totalNumber >= Rankings.TABLE_SIZE) {
				BitmapText total = createText( Utils.format(TXT_TOTAL, Rankings.INSTANCE.totalNumber), 8 );
			...
			BitmapText title = createText( TXT_NO_GAMES, 8 );
			title.hardlight( Window.TITLE_COLOR );
			title.measure();
			title.x = align( (w - title.width()) / 2 );
	...
	public static class Record extends Button {
		...
		protected void createChildren() {
			...
			position = new BitmapText(font1x);
			add( position );
